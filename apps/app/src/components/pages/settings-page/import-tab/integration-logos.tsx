import type { FC, SVGProps } from "react";

export const Obsidian = (props: SVGProps<SVGSVGElement>) => (
  <svg {...props} preserveAspectRatio="xMidYMid" viewBox="0 0 256 332">
    <title>Obsidian</title>
    <defs>
      <radialGradient
        cx="72.819%"
        cy="96.934%"
        fx="72.819%"
        fy="96.934%"
        gradientTransform="rotate(-104 11141.322 0)"
        id="obsidian__a"
        r="163.793%"
      >
        <stop offset="0%" stopColor="#FFF" stopOpacity=".4" />
        <stop offset="100%" stopOpacity=".1" />
      </radialGradient>
      <radialGradient
        cx="52.917%"
        cy="90.632%"
        fx="52.917%"
        fy="90.632%"
        gradientTransform="rotate(-82 10746.75 0)"
        id="obsidian__b"
        r="190.361%"
      >
        <stop offset="0%" stopColor="#FFF" stopOpacity=".6" />
        <stop offset="100%" stopColor="#FFF" stopOpacity=".1" />
      </radialGradient>
      <radialGradient
        cx="31.174%"
        cy="97.138%"
        fx="31.174%"
        fy="97.138%"
        gradientTransform="rotate(-77 10724.606 0)"
        id="obsidian__c"
        r="178.714%"
      >
        <stop offset="0%" stopColor="#FFF" stopOpacity=".8" />
        <stop offset="100%" stopColor="#FFF" stopOpacity=".4" />
      </radialGradient>
      <radialGradient
        cx="71.813%"
        cy="99.994%"
        fx="71.813%"
        fy="99.994%"
        gradientTransform="translate(0 22251839.658) skewY(-90)"
        id="obsidian__d"
        r="92.086%"
      >
        <stop offset="0%" stopColor="#FFF" stopOpacity=".3" />
        <stop offset="100%" stopOpacity=".3" />
      </radialGradient>
      <radialGradient
        cx="117.013%"
        cy="34.769%"
        fx="117.013%"
        fy="34.769%"
        gradientTransform="rotate(102 -1004.443 0)"
        id="obsidian__e"
        r="328.729%"
      >
        <stop offset="0%" stopColor="#FFF" stopOpacity="0" />
        <stop offset="100%" stopColor="#FFF" stopOpacity=".2" />
      </radialGradient>
      <radialGradient
        cx="-9.431%"
        cy="8.712%"
        fx="-9.431%"
        fy="8.712%"
        gradientTransform="rotate(45 1674.397 0)"
        id="obsidian__f"
        r="153.492%"
      >
        <stop offset="0%" stopColor="#FFF" stopOpacity=".2" />
        <stop offset="100%" stopColor="#FFF" stopOpacity=".4" />
      </radialGradient>
      <radialGradient
        cx="103.902%"
        cy="-22.172%"
        fx="103.902%"
        fy="-22.172%"
        gradientTransform="rotate(80 3757.522 0)"
        id="obsidian__g"
        r="394.771%"
      >
        <stop offset="0%" stopColor="#FFF" stopOpacity=".1" />
        <stop offset="100%" stopColor="#FFF" stopOpacity=".3" />
      </radialGradient>
      <radialGradient
        cx="99.348%"
        cy="89.193%"
        fx="99.348%"
        fy="89.193%"
        gradientTransform="translate(0 -38783246.548) skewY(-90)"
        id="obsidian__h"
        r="203.824%"
      >
        <stop offset="0%" stopColor="#FFF" stopOpacity=".2" />
        <stop offset="50%" stopColor="#FFF" stopOpacity=".2" />
        <stop offset="100%" stopColor="#FFF" stopOpacity=".3" />
      </radialGradient>
    </defs>
    <path
      d="M209.056 308.305c-2.043 14.93-16.738 26.638-31.432 22.552-20.823-5.658-44.946-14.616-66.634-16.266l-33.317-2.515a22.002 22.002 0 0 1-14.144-6.522L6.167 246.778a21.766 21.766 0 0 1-4.244-24.124s35.36-77.478 36.775-81.485c1.257-4.008 6.13-39.211 8.958-58.07a22.002 22.002 0 0 1 7.072-12.965L122.462 9.47a22.002 22.002 0 0 1 31.903 2.672l57.048 71.978a23.18 23.18 0 0 1 4.872 14.38c0 13.594 1.179 41.646 8.8 59.72a236.756 236.756 0 0 0 27.974 45.732 11.001 11.001 0 0 1 .786 12.258c-4.95 8.408-14.851 24.595-28.76 45.26a111.738 111.738 0 0 0-16.108 46.834h.079Z"
      fillOpacity=".3"
    />
    <path
      d="M209.606 305.79c-2.043 15.009-16.737 26.717-31.432 22.71-20.744-5.737-44.79-14.695-66.555-16.345L78.38 309.64a21.923 21.923 0 0 1-14.144-6.6L6.874 244.106a21.923 21.923 0 0 1-4.243-24.36s35.438-77.792 36.774-81.878c1.336-4.007 6.13-39.289 8.958-58.305a22.002 22.002 0 0 1 7.072-13.044L123.17 5.621a22.002 22.002 0 0 1 31.902 2.75l56.97 72.292a23.338 23.338 0 0 1 4.871 14.38c0 13.673 1.18 41.804 8.723 59.955a238.092 238.092 0 0 0 27.974 45.969 11.001 11.001 0 0 1 .864 12.336c-5.03 8.487-14.851 24.674-28.838 45.497a112.603 112.603 0 0 0-16.03 46.99Z"
      fill="#6C31E3"
    />
    <path
      d="M70.365 307.44c26.638-53.983 25.93-92.722 14.537-120.225-10.372-25.459-29.781-41.489-45.025-51.468a19.233 19.233 0 0 1-1.415 4.243L2.631 219.747a21.923 21.923 0 0 0 4.321 24.36l57.284 58.933a23.762 23.762 0 0 0 6.129 4.4Z"
      fill="url(#obsidian__a)"
    />
    <path
      d="M142.814 197.902a86.025 86.025 0 0 1 21.06 4.793c21.844 8.172 41.724 26.56 58.147 61.999 1.179-2.043 2.357-4.008 3.615-5.894a960.226 960.226 0 0 0 28.838-45.497 11.001 11.001 0 0 0-.786-12.336 238.092 238.092 0 0 1-28.052-45.969c-7.544-18.073-8.644-46.282-8.723-59.955 0-5.186-1.65-10.294-4.871-14.38l-56.97-72.292-.943-1.178c4.165 13.75 3.93 24.752 1.336 34.731-2.357 9.272-6.757 17.68-11.394 26.56-1.571 2.986-3.143 6.05-4.636 9.193a110.01 110.01 0 0 0-12.415 45.576c-.786 19.016 3.064 42.825 15.716 74.65h.078Z"
      fill="url(#obsidian__b)"
    />
    <path
      d="M142.736 197.902c-12.652-31.824-16.502-55.633-15.716-74.65.786-18.858 6.286-33.002 12.415-45.575l4.715-9.193c4.558-8.88 8.88-17.288 11.315-26.56a61.684 61.684 0 0 0-1.336-34.731c-8.136-8.94-21.96-9.642-30.96-1.572L55.436 66.519a22.002 22.002 0 0 0-7.072 13.044l-8.25 54.69c0 .55-.158 1.022-.236 1.572 15.244 9.901 34.574 25.931 45.025 51.312 2.043 5.029 3.772 10.294 5.029 16.03a157.157 157.157 0 0 1 52.805-5.343v.078Z"
      fill="url(#obsidian__c)"
    />
    <path
      d="M178.253 328.5c14.616 4.007 29.31-7.701 31.353-22.789a120.225 120.225 0 0 1 12.494-41.017c-16.502-35.44-36.382-53.827-58.148-61.999-23.18-8.643-48.404-5.736-74.021.472 5.736 26.01 2.357 60.034-19.487 104.273 2.436 1.257 5.186 1.965 7.936 2.2l34.496 2.593c18.701 1.336 46.597 11.001 65.377 16.266Z"
      fill="url(#obsidian__d)"
    />
    <path
      d="M127.177 122.074c-.864 18.859 1.493 40.39 14.144 72.135l-3.929-.393c-11.394-33.081-13.908-50.054-13.044-69.149.786-19.094 6.994-33.789 13.123-46.361 1.571-3.143 5.186-9.037 6.758-12.023 4.557-8.879 7.622-13.515 10.215-21.609 3.772-11.315 2.986-16.658 2.514-22.001 2.908 19.251-8.172 35.988-16.501 53.04a113.939 113.939 0 0 0-13.358 46.361h.078Z"
      fill="url(#obsidian__e)"
    />
    <path
      d="M88.674 188.551c1.571 3.458 2.907 6.287 3.85 10.608l-3.379.786c-1.336-5.029-2.357-8.643-4.322-12.965-11.472-26.953-29.86-40.861-44.79-51.076 18.074 9.744 36.697 25.066 48.64 52.647Z"
      fill="url(#obsidian__f)"
    />
    <path
      d="M92.681 202.617c6.286 29.467-.786 66.948-21.609 103.409 17.445-36.146 25.931-70.8 18.859-102.938l2.75-.55v.079Z"
      fill="url(#obsidian__g)"
    />
    <path
      d="M164.659 199.867c34.181 12.808 47.383 40.86 57.205 64.355-12.18-24.516-29.074-51.626-58.462-61.684-22.317-7.7-41.175-6.758-73.471.55l-.707-3.143c34.26-7.858 52.176-8.8 75.435 0v-.078Z"
      fill="url(#obsidian__h)"
    />
  </svg>
);

export const Notion = (props: SVGProps<SVGSVGElement>) => (
  <svg {...props} preserveAspectRatio="xMidYMid" viewBox="0 0 256 268">
    <title>Notion</title>
    <path
      d="M16.092 11.538 164.09.608c18.179-1.56 22.85-.508 34.28 7.801l47.243 33.282C253.406 47.414 256 48.975 256 55.207v182.527c0 11.439-4.155 18.205-18.696 19.24L65.44 267.378c-10.913.517-16.11-1.043-21.825-8.327L8.826 213.814C2.586 205.487 0 199.254 0 191.97V29.726c0-9.352 4.155-17.153 16.092-18.188Z"
      fill="#FFF"
    />
    <path d="M164.09.608 16.092 11.538C4.155 12.573 0 20.374 0 29.726v162.245c0 7.284 2.585 13.516 8.826 21.843l34.789 45.237c5.715 7.284 10.912 8.844 21.825 8.327l171.864-10.404c14.532-1.035 18.696-7.801 18.696-19.24V55.207c0-5.911-2.336-7.614-9.21-12.66l-1.185-.856L198.37 8.409C186.94.1 182.27-.952 164.09.608ZM69.327 52.22c-14.033.945-17.216 1.159-25.186-5.323L23.876 30.778c-2.06-2.086-1.026-4.69 4.163-5.207l142.274-10.395c11.947-1.043 18.17 3.12 22.842 6.758l24.401 17.68c1.043.525 3.638 3.637.517 3.637L71.146 52.095l-1.819.125Zm-16.36 183.954V81.222c0-6.767 2.077-9.887 8.3-10.413L230.02 60.93c5.724-.517 8.31 3.12 8.31 9.879v153.917c0 6.767-1.044 12.49-10.387 13.008l-161.487 9.361c-9.343.517-13.489-2.594-13.489-10.921ZM212.377 89.53c1.034 4.681 0 9.362-4.681 9.897l-7.783 1.542v114.404c-6.758 3.637-12.981 5.715-18.18 5.715-8.308 0-10.386-2.604-16.609-10.396l-50.898-80.079v77.476l16.1 3.646s0 9.362-12.989 9.362l-35.814 2.077c-1.043-2.086 0-7.284 3.63-8.318l9.351-2.595V109.823l-12.98-1.052c-1.044-4.68 1.55-11.439 8.826-11.965l38.426-2.585 52.958 81.113v-71.76l-13.498-1.552c-1.043-5.733 3.111-9.896 8.3-10.404l35.84-2.087Z" />
  </svg>
);

export const Chrome = (props: SVGProps<SVGSVGElement>) => (
  <svg {...props} preserveAspectRatio="xMidYMid" viewBox="0 0 190.5 190.5">
    <title>Google Chrome</title>
    <path
      d="M95.252 142.873c26.304 0 47.627-21.324 47.627-47.628s-21.323-47.628-47.627-47.628-47.627 21.324-47.627 47.628 21.323 47.628 47.627 47.628z"
      fill="#fff"
    />
    <path
      d="m54.005 119.07-41.24-71.43a95.227 95.227 0 0 0-.003 95.25 95.234 95.234 0 0 0 82.496 47.61l41.24-71.43v-.011a47.613 47.613 0 0 1-17.428 17.443 47.62 47.62 0 0 1-47.632.007 47.62 47.62 0 0 1-17.433-17.437z"
      fill="#229342"
    />
    <path
      d="m136.495 119.067-41.239 71.43a95.229 95.229 0 0 0 82.489-47.622A95.24 95.24 0 0 0 190.5 95.248a95.237 95.237 0 0 0-12.772-47.623H95.249l-.01.007a47.62 47.62 0 0 1 23.819 6.372 47.618 47.618 0 0 1 17.439 17.431 47.62 47.62 0 0 1-.001 47.633z"
      fill="#fbc116"
    />
    <path
      d="M95.252 132.961c20.824 0 37.705-16.881 37.705-37.706S116.076 57.55 95.252 57.55 57.547 74.431 57.547 95.255s16.881 37.706 37.705 37.706z"
      fill="#1a73e8"
    />
    <path
      d="M95.252 47.628h82.479A95.237 95.237 0 0 0 142.87 12.76 95.23 95.23 0 0 0 95.245 0a95.222 95.222 0 0 0-47.623 12.767 95.23 95.23 0 0 0-34.856 34.872l41.24 71.43.011.006a47.62 47.62 0 0 1-.015-47.633 47.61 47.61 0 0 1 41.252-23.815z"
      fill="#e33b2e"
    />
  </svg>
);

export const Safari = (props: SVGProps<SVGSVGElement>) => (
  <svg {...props} viewBox="0 0 66.165833 65.803795">
    <title>Safari</title>
    <defs>
      <linearGradient id="safari__b">
        <stop offset="0" stopColor="#06c2e7" />
        <stop offset=".25000015" stopColor="#0db8ec" />
        <stop offset=".5000003" stopColor="#12aef1" />
        <stop offset=".75000012" stopColor="#1f86f9" />
        <stop offset="1" stopColor="#107ddd" />
      </linearGradient>
      <linearGradient id="safari__a">
        <stop offset="0" stopColor="#bdbdbd" />
        <stop offset="1" stopColor="#fff" />
      </linearGradient>
      <linearGradient
        gradientTransform="translate(206.79018 159.77261) scale(.35154)"
        gradientUnits="userSpaceOnUse"
        id="safari__d"
        x1="412.97501"
        x2="412.97501"
        xlinkHref="#safari__a"
        y1="237.60777"
        y2="59.392235"
      />
      <radialGradient
        cx="413.06128"
        cy="136.81819"
        fx="413.06128"
        fy="136.81819"
        gradientTransform="translate(194.54473 155.58044) scale(.38143)"
        gradientUnits="userSpaceOnUse"
        id="safari__e"
        r="82.125351"
        xlinkHref="#safari__b"
      />
    </defs>
    <path
      d="M383.29373 211.97671a31.325188 31.325188 0 0 1-31.32519 31.32519 31.325188 31.325188 0 0 1-31.32518-31.32519 31.325188 31.325188 0 0 1 31.32518-31.32519 31.325188 31.325188 0 0 1 31.32519 31.32519z"
      fill="url(#safari__d)"
      stroke="#cdcdcd"
      strokeLinecap="round"
      strokeLinejoin="round"
      strokeWidth=".09301235"
      transform="translate(-318.88562 -180.59501)"
    />
    <path
      d="M380.83911 211.97671a28.870571 28.870571 0 0 1-28.87057 28.87057 28.870571 28.870571 0 0 1-28.87057-28.87057 28.870571 28.870571 0 0 1 28.87057-28.87057 28.870571 28.870571 0 0 1 28.87057 28.87057z"
      fill="url(#safari__e)"
      transform="translate(-318.88562 -180.59501)"
    />
    <path
      d="m36.3834003 34.83806178-6.60095092-6.91272438 23.41607429-15.75199774z"
      fill="#ff5150"
    />
    <path
      d="m36.38339038 34.83805895-6.60095092-6.91272438-16.81512624 22.66471911z"
      fill="#f1f1f1"
    />
    <path
      d="m12.96732 50.59006 23.41607-15.75201 16.81513-22.66472z"
      opacity=".243"
    />
  </svg>
);

export const Firefox = (props: SVGProps<SVGSVGElement>) => (
  <svg {...props} preserveAspectRatio="xMidYMid" viewBox="0 0 256 265">
    <title>Firefox</title>
    <defs>
      <radialGradient
        cx="-7907.187"
        cy="-8515.121"
        gradientTransform="translate(26367.938 28186.305) scale(3.3067)"
        gradientUnits="userSpaceOnUse"
        id="firefox__b"
        r="80.797"
      >
        <stop offset=".129" stopColor="#ffbd4f" />
        <stop offset=".186" stopColor="#ffac31" />
        <stop offset=".247" stopColor="#ff9d17" />
        <stop offset=".283" stopColor="#ff980e" />
        <stop offset=".403" stopColor="#ff563b" />
        <stop offset=".467" stopColor="#ff3750" />
        <stop offset=".71" stopColor="#f5156c" />
        <stop offset=".782" stopColor="#eb0878" />
        <stop offset=".86" stopColor="#e50080" />
      </radialGradient>
      <radialGradient
        cx="-7945.648"
        cy="-8460.984"
        gradientTransform="translate(26367.938 28186.305) scale(3.3067)"
        gradientUnits="userSpaceOnUse"
        id="firefox__e"
        r="38.471"
      >
        <stop offset=".353" stopColor="#3a8ee6" />
        <stop offset=".472" stopColor="#5c79f0" />
        <stop offset=".669" stopColor="#9059ff" />
        <stop offset="1" stopColor="#c139e6" />
      </radialGradient>
      <linearGradient
        gradientTransform="translate(-2.999 -.01) scale(3.3067)"
        gradientUnits="userSpaceOnUse"
        id="firefox__a"
        x1="70.786"
        x2="6.447"
        y1="12.393"
        y2="74.468"
      >
        <stop offset=".048" stopColor="#fff44f" />
        <stop offset=".111" stopColor="#ffe847" />
        <stop offset=".225" stopColor="#ffc830" />
        <stop offset=".368" stopColor="#ff980e" />
        <stop offset=".401" stopColor="#ff8b16" />
        <stop offset=".462" stopColor="#ff672a" />
        <stop offset=".534" stopColor="#ff3647" />
        <stop offset=".705" stopColor="#e31587" />
      </linearGradient>
    </defs>
    <path
      d="M248.033 88.713c-5.569-13.399-16.864-27.866-25.71-32.439a133.169 133.169 0 0 1 12.979 38.9l.023.215c-14.49-36.126-39.062-50.692-59.13-82.41a155.1 155.1 0 0 1-3.019-4.907 40.605 40.605 0 0 1-1.412-2.645 23.31 23.31 0 0 1-1.912-5.076.331.331 0 0 0-.291-.331.469.469 0 0 0-.241 0c-.016 0-.043.03-.063.037-.02.006-.063.036-.092.049l.049-.086c-32.19 18.849-43.113 53.741-44.118 71.194a64.108 64.108 0 0 0-35.269 13.593 38.336 38.336 0 0 0-3.307-2.506 59.417 59.417 0 0 1-.36-31.324 94.912 94.912 0 0 0-30.848 23.841h-.06c-5.079-6.438-4.722-27.667-4.431-32.102a22.957 22.957 0 0 0-4.279 2.272 93.435 93.435 0 0 0-12.526 10.73 111.954 111.954 0 0 0-11.98 14.375v.019-.023A108.26 108.26 0 0 0 4.841 108.92l-.171.846a203.818 203.818 0 0 0-1.26 8.003c0 .096-.02.185-.03.281a122.12 122.12 0 0 0-2.08 17.667v.662c.086 98.661 106.944 160.23 192.344 110.825a128.165 128.165 0 0 0 62.12-89.153c.215-1.653.39-3.29.582-4.96a131.8 131.8 0 0 0-8.313-64.378ZM100.322 189.031c.599.288 1.161.599 1.776.873l.089.057a68.838 68.838 0 0 1-1.865-.93Zm29.357-77.297Zm105.656-16.315v-.123l.023.136-.023-.013Z"
      fill="url(#firefox__a)"
    />
    <path
      d="M248.033 88.713c-5.569-13.399-16.864-27.866-25.71-32.439a133.169 133.169 0 0 1 12.979 38.9v.122l.023.136a116.067 116.067 0 0 1-3.988 86.497c-14.688 31.516-50.242 63.819-105.894 62.248-60.132-1.703-113.089-46.323-122.989-104.766-1.802-9.216 0-13.888.906-21.378a95.444 95.444 0 0 0-2.06 17.684v.662c.086 98.661 106.944 160.23 192.344 110.825a128.165 128.165 0 0 0 62.12-89.153c.215-1.653.39-3.29.582-4.96a131.8 131.8 0 0 0-8.313-64.378Z"
      fill="url(#firefox__b)"
    />
    <path
      d="M129.683 111.734c-.212 3.188-11.475 14.182-15.413 14.182-36.443 0-42.359 22.046-42.359 22.046 1.614 18.564 14.55 33.854 30.187 41.942.714.371 1.439.705 2.163 1.032a70.572 70.572 0 0 0 3.763 1.541 56.974 56.974 0 0 0 16.675 3.217c63.876 2.996 76.25-76.384 30.154-99.419a44.241 44.241 0 0 1 30.901 7.503A64.68 64.68 0 0 0 129.6 70.985c-1.521 0-3.009.126-4.504.229a64.108 64.108 0 0 0-35.269 13.593c1.954 1.654 4.16 3.863 8.806 8.442 8.696 8.568 31 17.443 31.05 18.485Z"
      fill="url(#firefox__e)"
    />
  </svg>
);

interface LogoProps {
  "aria-hidden"?: boolean | "true" | "false";
  className?: string;
}

export type LogoComponent = FC<LogoProps>;

function RasterLogo({
  alt,
  src,
  className,
  ...props
}: LogoProps & { alt: string; src: string }) {
  return (
    // biome-ignore lint/correctness/useImageSize: <>
    <img
      alt={alt}
      className={`${className ?? ""} rounded-sm object-contain`.trim()}
      src={src}
      {...props}
    />
  );
}

export const Fabric: LogoComponent = ({ className, ...props }) => (
  <svg
    {...props}
    className={`[&_path]:fill-black [&_path]:stroke-black dark:[&_path]:fill-white dark:[&_path]:stroke-white ${className ? ` ${className}` : ""}`}
    preserveAspectRatio="xMidYMid meet"
    viewBox="0 2 31 28"
    xmlns="http://www.w3.org/2000/svg"
  >
    <title>Fabric</title>
    <path
      d="M 10.442 2.735 C 11.412 2.735 12.321 3.207 12.88 3.999 L 14.219 5.9 L 25.802 5.9 C 28.373 5.9 30.458 7.985 30.458 10.556 L 30.458 24.973 C 30.458 27.545 28.373 29.63 25.802 29.63 L 4.872 29.63 C 2.3 29.63 0.215 27.545 0.215 24.973 L 0.215 7.392 C 0.215 4.82 2.3 2.735 4.872 2.735 Z M 4.872 4.253 C 3.138 4.253 1.732 5.658 1.732 7.392 L 1.732 24.973 C 1.732 26.707 3.138 28.113 4.872 28.113 L 25.802 28.113 C 27.535 28.113 28.941 26.707 28.941 24.973 L 28.941 10.556 C 28.941 8.823 27.535 7.417 25.802 7.417 L 13.54 7.417 C 13.472 7.417 13.409 7.384 13.369 7.328 L 11.639 4.873 C 11.365 4.484 10.918 4.252 10.442 4.253 Z"
      fill="currentColor"
      stroke="currentColor"
      strokeLinecap="round"
      strokeLinejoin="round"
      strokeWidth="0.34"
    />
    <path
      d="M 10.257 21.459 C 10.308 21.514 10.361 21.568 10.415 21.622 C 13.202 24.407 17.721 24.407 20.509 21.622 C 20.562 21.569 20.614 21.515 20.665 21.461 L 19.671 20.468 C 19.62 20.523 19.568 20.577 19.515 20.63 C 17.276 22.867 13.646 22.867 11.407 20.63 C 11.354 20.577 11.301 20.522 11.25 20.467 Z M 8.869 12.116 L 10.48 12.116 L 10.48 15.796 L 8.869 15.796 Z"
      fill="currentColor"
      stroke="currentColor"
      strokeMiterlimit="10"
      strokeWidth="0.34"
    />
    <path
      d="M 15.312 13.037 L 16.694 13.037 L 16.694 17.637 L 16.695 17.637 L 16.695 19.017 L 13.932 19.017 L 13.932 17.637 L 15.312 17.637 L 15.312 13.037 Z"
      fill="currentColor"
      stroke="currentColor"
      strokeWidth={0.5}
    />
    <path
      d="M 16.694 13.037 L 16.866 13.037 L 16.866 12.865 L 16.694 12.865 Z M 15.312 13.037 L 15.312 12.865 L 15.141 12.865 L 15.141 13.037 Z M 16.694 17.637 L 16.522 17.637 L 16.522 17.809 L 16.694 17.809 L 16.694 17.637 Z M 16.695 17.637 L 16.867 17.637 L 16.867 17.465 L 16.695 17.465 Z M 16.695 19.017 L 16.695 19.189 L 16.867 19.189 L 16.867 19.017 Z M 13.932 19.017 L 13.76 19.017 L 13.76 19.189 L 13.932 19.189 Z M 13.932 17.637 L 13.932 17.465 L 13.76 17.465 L 13.76 17.637 Z M 15.312 17.637 L 15.312 17.809 L 15.485 17.809 L 15.485 17.637 Z M 16.694 12.865 L 15.312 12.865 L 15.312 13.209 L 16.694 13.209 Z M 16.866 13.527 L 16.866 13.037 L 16.522 13.037 L 16.522 13.527 L 16.866 13.527 Z M 16.866 17.637 L 16.866 13.528 L 16.522 13.528 L 16.522 17.637 L 16.866 17.637 Z M 16.694 17.809 L 16.695 17.809 L 16.695 17.465 L 16.694 17.465 L 16.694 17.809 Z M 16.523 17.637 L 16.523 19.017 L 16.867 19.017 L 16.867 17.637 L 16.523 17.637 Z M 16.695 18.845 L 13.932 18.845 L 13.932 19.189 L 16.695 19.189 L 16.695 18.845 Z M 14.105 19.017 L 14.105 17.637 L 13.76 17.637 L 13.76 19.017 L 14.105 19.017 Z M 13.932 17.809 L 15.312 17.809 L 15.312 17.465 L 13.932 17.465 Z M 15.14 13.527 L 15.14 17.637 L 15.485 17.637 L 15.485 13.528 L 15.141 13.528 Z M 15.14 13.037 L 15.14 13.527 L 15.485 13.527 L 15.485 13.037 L 15.141 13.037 Z"
      fill="currentColor"
      stroke="currentColor"
      strokeWidth={0.5}
    />
    <path
      d="M 20.151 12.116 L 21.762 12.116 L 21.762 15.796 L 20.151 15.796 Z"
      fill="currentColor"
      stroke="currentColor"
      strokeMiterlimit="10"
      strokeWidth="0.34"
    />
  </svg>
);

export const Raindrop: LogoComponent = (props) => (
  <svg
    {...props}
    viewBox="0 0 48 48"
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
  >
    <title>Raindrop</title>
    <defs>
      <path
        d="M12 0a12 12 0 0 1 12 12v12H12a12 12 0 1 1 0-24z"
        id="raindrop__a"
      />
      <path d="M0 24V11.7A12 12 0 1 1 12 24H0z" id="raindrop__c" />
    </defs>
    <g fill="none" fillRule="evenodd">
      <path
        d="M35.3 9.7a16 16 0 0 1-.6 23.2L24 43 13.3 33l-.6-.7A16 16 0 1 1 35.3 9.7z"
        fill="#1988E0"
      />
      <g transform="translate(0 19)">
        <mask fill="#fff" id="raindrop__b">
          <use xlinkHref="#raindrop__a" />
        </mask>
        <use fill="#2CD4ED" xlinkHref="#raindrop__a" />
        <path
          d="M35.3-9.3a16 16 0 0 1-.6 23.2L24 24 13.3 14l-.6-.7A16 16 0 1 1 35.3-9.3z"
          fill="#0DB4E2"
          mask="url(#raindrop__b)"
        />
      </g>
      <g transform="translate(24 19)">
        <mask fill="#fff" id="raindrop__d">
          <use xlinkHref="#raindrop__c" />
        </mask>
        <use fill="#3169FF" xlinkHref="#raindrop__c" />
        <path
          d="M11.3-9.3a16 16 0 0 1-.6 23.2L0 24l-10.7-10-.6-.7A16 16 0 1 1 11.3-9.3z"
          fill="#3153FF"
          mask="url(#raindrop__d)"
        />
      </g>
    </g>
  </svg>
);

export const Logseq: LogoComponent = (props) => (
  <RasterLogo alt="Logseq" src="/import-logos/logseq.png" {...props} />
);

export const Bear: LogoComponent = (props) => (
  <RasterLogo alt="Bear" src="/import-logos/bear.png" {...props} />
);

export const Evernote: LogoComponent = (props) => (
  <RasterLogo alt="Evernote" src="/import-logos/evernote.jpg" {...props} />
);

export const Readwise: LogoComponent = (props) => (
  <RasterLogo alt="Readwise" src="/import-logos/readwise.svg" {...props} />
);

export const Pocket: LogoComponent = (props) => (
  <RasterLogo alt="Pocket" src="/import-logos/pocket.png" {...props} />
);

export const Instapaper: LogoComponent = (props) => (
  <RasterLogo alt="Instapaper" src="/import-logos/instapaper.png" {...props} />
);

export const MyMind: LogoComponent = (props) => (
  <RasterLogo alt="MyMind" src="/import-logos/mymind.png" {...props} />
);

export const GitHub = (props: SVGProps<SVGSVGElement>) => (
  <svg {...props} fill="none" viewBox="0 0 1024 1024">
    <title>GitHub</title>
    <path
      clipRule="evenodd"
      d="M8 0C3.58 0 0 3.58 0 8C0 11.54 2.29 14.53 5.47 15.59C5.87 15.66 6.02 15.42 6.02 15.21C6.02 15.02 6.01 14.39 6.01 13.72C4 14.09 3.48 13.23 3.32 12.78C3.23 12.55 2.84 11.84 2.5 11.65C2.22 11.5 1.82 11.13 2.49 11.12C3.12 11.11 3.57 11.7 3.72 11.94C4.44 13.15 5.59 12.81 6.05 12.6C6.12 12.08 6.33 11.73 6.56 11.53C4.78 11.33 2.92 10.64 2.92 7.58C2.92 6.71 3.23 5.99 3.74 5.43C3.66 5.23 3.38 4.41 3.82 3.31C3.82 3.31 4.49 3.1 6.02 4.13C6.66 3.95 7.34 3.86 8.02 3.86C8.7 3.86 9.38 3.95 10.02 4.13C11.55 3.09 12.22 3.31 12.22 3.31C12.66 4.41 12.38 5.23 12.3 5.43C12.81 5.99 13.12 6.7 13.12 7.58C13.12 10.65 11.25 11.33 9.47 11.53C9.76 11.78 10.01 12.26 10.01 13.01C10.01 14.08 10 14.94 10 15.21C10 15.42 10.15 15.67 10.55 15.59C13.71 14.53 16 11.53 16 8C16 3.58 12.42 0 8 0Z"
      fill="#ffff"
      fillRule="evenodd"
      transform="scale(64)"
    />
  </svg>
);

export const INTEGRATION_LOGO: Record<string, LogoComponent> = {
  obsidian: Obsidian,
  logseq: Logseq,
  bear: Bear,
  notion_zip: Notion,
  notion_oauth: Notion,
  evernote: Evernote,
  readwise: Readwise,
  raindrop_oauth: Raindrop,
  raindrop_csv: Raindrop,
  pocket: Pocket,
  instapaper: Instapaper,
  mymind: MyMind,
  chrome: Chrome,
  safari: Safari,
  firefox: Firefox,
  fabric: Fabric,
  github: GitHub,
};
